...
public class LeaveBalanceResetService {
        ...
        try {
            ...
            for (Employee employee : employees) {
                ...
                if (today.isEqual(joinedDate.plusYears(1))) {
                    log.info("[LEAVE-BALANCE-RESET] Employee eligible for leave balance reset: {}", employee.getEmployeeId());
    ...
    private void resetLeaveBalanceForEmployee(Employee employee) {
        ...
        try {
            ...
            for (LeaveBalance leaveBalance : leaveBalances) {
                switch (leaveBalance.getLeaveType()) {
                    ...
                    case "Casual Leave":
                        ...
                        break;
                    default:
                        log.warn("[LEAVE-BALANCE-RESET] Unknown leave type '{}' for employee ID: {}", leaveBalance.getLeaveType(), employee.getEmployeeId());
                        continue;
                }
            ...
            log.error("[LEAVE-BALANCE-RESET] Error resetting leave balances for employee ID: {}. Error: {}", employee.getEmployeeId(), e.getMessage(), e);
        }
    }
}
