...
import com.cognizant.employeemanagement.repository.ShiftRequestRepository;
	...
	public ReturnSwapRequestDTO approveShiftSwap(int requestId, boolean status) {
		...
		if (status) {
			...
			Employee existingEmployee = employeeRepository.findById(shiftRequest.getEmployee().getEmployeeId())
					.orElseThrow(() -> {
						log.warn("[EMPLOYEE-SERVICE] Employee not found with ID: {}",
								shiftRequest.getEmployee().getEmployeeId());
						return new RuntimeException(
								"Employee not found with ID: " + shiftRequest.getEmployee().getEmployeeId());
					});

			Shift shift = shiftRepository.findById(shiftRequest.getRequestedShift().getShiftId()).orElseThrow(() -> {
				log.warn("[EMPLOYEE-SERVICE] Invalid shift ID: {}", shiftRequest.getRequestedShift().getShiftId());
				return new IllegalArgumentException(
						"Invalid Shift ID: " + shiftRequest.getRequestedShift().getShiftId());
			});
